...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        if scale_factor == 4:
            ...
            raise ValueError(f'scale_factor={scale_factor} , not supported')
    ...
    def eval_dataset(self, data_loader: torch.utils.data.DataLoader, time_steps=None):
        ...
        for data_batch in tqdm(data_loader):
            ...
            full_recons, direct_recons, img_times, recon_from_time = self.sample(batch_size=batch_size,
                                                                img_start=data_batch,
                                                                time_steps=time_steps)
            l2_full_recon_ = [F.mse_loss(src, tgt).cpu().numpy() for src, tgt in zip(img_times, full_recons)]
            l2_direct_recon_ = [F.mse_loss(src, tgt).cpu().numpy() for src, tgt in (img_times, direct_recons)]
            l2_from_recon_ = [F.mse_loss(tgt, data_batch).cpu().numpy() for tgt in direct_recons]
            ...
            l2_from_recon.append(np.array(l2_from_recon_))
        ...
        l2_from_recon = np.array(l2_from_recon).mean(0)

        return l2_full_recon, l2_direct_recon, l2_times, l2_from_recon
    ...
    def sample(self, batch_size = 16, img_start=None, time_steps=None):
        ...
        # 3(2), 2(1), 1(0)
        # xt = img
        direct_recons = [self.denoise_fn(img_times[t],  torch.full((batch_size,), t, dtype=torch.long).cuda())
                         for t in range(1, time_steps)]

        full_recons = self.backward_recon(img_times[-1], time_steps, batch_size)

        recon_from_time = [self.backward_recon(img_times[t], time_steps, batch_size)[0] for t in range(1, time_steps)]

        return full_recons, direct_recons, img_times, recon_from_time

    def backward_recon(self, img, time_steps, batch_size):
        full_recons = []
    ...
    def p_losses(self, x_start, t):
        ...
        elif self.train_routine == 'Step' or self.train_routine == 'Step_t':
            x_t = self.q_sample(x_start=x_start, t=t) # --> t
            x_t_sub = self.q_sample(x_start=x_start, t=t-1) # -->t-1

            x_t_sub_pred = self.denoise_fn(x_t, t) # t --> t-1
        ...
        #     x_t = self.q_sample(x_start=x_start, t=t) # --> t
        #     x_t_sub = self.q_sample(x_start=x_start, t=t-1) # --> t-1
        #     x_t_sub_sub = self.q_sample(x_start=x_start, t=t - 2) # --> t-2
        #
        #     x_t_sub_pred = self.denoise_fn(x_t, t) # t--> t-1
        #     x_t_sub_pred_t = self.denoise_fn(x_t_sub_pred, t-1)# t-1--> t-2
        #
        #     if self.loss_type == 'l1':
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.log = logging.getLogger('Train-res-diff')
        log_dir = os.path.join(results_folder, 'logs')
        os.makedirs(log_dir, exist_ok=True)
        self._tensorboard = tensorboardX.SummaryWriter(log_dir, flush_secs=60)
        ...
        ds_val = torch.utils.data.Subset(Dataset(folder, image_size), list(range(int(len(self.ds)*0.005))))
        ...
        while self.step < self.train_num_steps:
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                if self.step % log_loss_tr_every == 0:
                    self.log.info(f'step={self.step}, grad_step={i} loss={float(loss.item()): .4f}')
                    self._tensorboard.add_scalar(f"loss_tr_grad_{i}/train", loss, self.step)
                u_loss += loss.item()
            ...
            if self.step % self.save_and_sample_every == 0:
                ...
                full_recons, direct_recons, img_times, recon_from_time = self.ema_model.module.sample(batch_size=val_batch.shape[0],
                                                                                     img_start=val_batch)
                ...
                for n_time in range(len(img_times)):
                    if n_time < len(full_recons):
                        ...
                        self._tensorboard.add_images(f'full_recon_t_{n_time}', recon_img, self.step)
                    ...
                    if n_time < len(direct_recons):
                        ...
                        self._tensorboard.add_images(f'direct_recon_t_{n_time}', direct_recon_img, self.step)

                    if n_time < len(img_times):
                        xt = (img_times[n_time] + 1) * 0.5
                        xt_file = os.path.join(res_dir, f'sample-xt-{milestone}-t_{n_time}.png')
                        utils.save_image(xt, xt_file, nrow=6)
                        self._tensorboard.add_images(f'x_t_{n_time}', xt, self.step)

                    if n_time < len(recon_from_time):
                        recon_from_t = (recon_from_time[n_time] + 1) * 0.5
                        recon_from_file = os.path.join(res_dir, f'recon_from-xt-{milestone}-t_{n_time}.png')
                        utils.save_image(recon_from_t, recon_from_file, nrow=6)
                        self._tensorboard.add_images(f'recon_from_t_{n_time}', recon_from_t, self.step)

                err_seq_to_str = lambda err_seq: ' '.join(list(map(lambda err_: f'{err_: .4f}', err_seq)))
                def add_steps_metric(name, tag, metric_list):
                    for step, val in enumerate(metric_list):
                        self._tensorboard.add_scalar(f"{name}_step_{step}/{tag}", val, self.step)

                self.log.info(f"Run Eval step: {self.step}")
                l2_full_recon, l2_direct_recon, l2_times, l2_from_recon = self.ema_model.module.eval_dataset(self.data_loader_val)
                self.log.info(f"Eval data: step: {self.step} l2_full_recon: {err_seq_to_str(l2_full_recon)}")
                add_steps_metric('l2_full_recon', 'valid', l2_full_recon)

                self.log.info(f"Eval data: step: {self.step} l2_direct_recon: {err_seq_to_str(l2_direct_recon)}")
                add_steps_metric('l2_direct_recon', 'valid', l2_direct_recon)

                self.log.info(f"Eval data: step: {self.step} l2_times: {err_seq_to_str(l2_times)}")
                add_steps_metric('l2_times', 'valid', l2_times)

                self.log.info(f"Eval data: step: {self.step} l2_from_recon: {err_seq_to_str(l2_from_recon)}")
                add_steps_metric('l2_from_recon', 'valid', l2_from_recon)

                acc_loss = acc_loss/(self.save_and_sample_every+1)
                self.log.info(f'Mean of last {self.step}: {acc_loss: .4f}')
                self._tensorboard.add_scalar(f"acc_loss/train", acc_loss, self.step)
                acc_loss=0
